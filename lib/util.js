...
exports.linkifyText = function (input, opts) {
  ...
  // Looks for URLs
  let linkedAndEmbedded = text.split(urlRegex()).reduce((frag, text, index) => {
    if (index % 2) { // URLs are always in odd positions
      ...
      // must end with jpg, png, gif, or svg
  ...
  // Attach to dummy element
  const dummy = document.createElement('div')
  dummy.appendChild(linkedAndEmbedded.cloneNode(true))
  let parsedHtml = dummy.innerHTML

  // Looks for mentions
  if (input.hasOwnProperty('mentions')) {
    input.mentions.map(mention => {
      // find @names, replaces with links to relative url
      parsedHtml = parsedHtml.replace(new RegExp(`@${ mention.name }`, 'g'), `<a href="/user/${ mention.url }">@${ mention.name }</a>`)
    })
  }

  return document.createRange().createContextualFragment(parsedHtml);
}

exports.buildPost = function (startingValue) {
